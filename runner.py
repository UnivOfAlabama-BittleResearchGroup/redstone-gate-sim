...
DETECTOR_IDS = [["".join(['detect_', str(i)]) for i in range(0, GATE_LANE_NUM)],
                ...
                ["automated_gate_detect", "automated_gate_reject_detect"]]
...
EXIT_EDGE = 'exit_edge'
ACCEPTABLE_IDS = ["".join(['routeFlow_1.', str(random.randint(0, 50))]) for i in range(30)]
...
def run(detector_ids, sim_step, sim_length, lane_num, traffic_light_id, control_edge, automated_gate_edge,
    ...
    while traci.simulation.getTime() < sim_length:
        ...
        rejected_vehicle = automatic_gate_manager.manage(detector_state=automated_detect)
        if rejected_vehicle:
            vehicle_manager.reroute_rejected(rejected_vehicle)
        # sim step
...
if __name__ == "__main__":
    ...
    run(detector_ids=DETECTOR_IDS, sim_step=SIM_STEP, sim_length=SIM_LENGTH, traffic_light_id=TRAFFIC_LIGHT_ID,
        ...
        automated_tl_id=AUTOMATED_TL_ID, acceptable_ids=ACCEPTABLE_IDS, rejected_edge=EXIT_EDGE)
